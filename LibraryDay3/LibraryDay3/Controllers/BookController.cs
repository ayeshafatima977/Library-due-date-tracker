    ...
    public class BookController : Controller
    {
        ...
        public IActionResult Create(string title, string authorID, string publicationDate)
        {
            ...
            if (Request.Query.Count > 0)
                try
                {
                    var createBook = CreateBook(title, int.Parse(authorID), publicationDate);
        ...
        public Book CreateBook(string title, int authorID, string publicationDate)
        {
            ...
            if (exception.newExceptions.Count > 0) throw exception;
        ...
        public Book GetBookByID(int id)
        {
            ...
            using (var context = new LibraryContext())
            {
                getBook = context.Books.Where(x => x.ID == id).Include(x => x.Author).Include(x => x.Borrows)
                    .SingleOrDefault();
            }
        ...
        {
            var overdueList = new List<Borrow>();
            ...
            foreach (var borrow in overdueList)
